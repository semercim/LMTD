...
class CLMTD(LMTD):
    ...
    def __init__(self, num_components=None, max_iter=100, dimensions=None,
        ...
        """
        super(CLMTD, self).__init__(num_components, max_iter, dimensions, mu, beta, gamma, alpha,
    ...
    def calculate_loss(self, tensors_list, core_tensors, target_neighbors):
        ...
        for index in range(num_instances):
            ...
            for target in target_neighbors[index, :]:
                loss2 += np.sum((vectorized_tensors[index] - vectorized_tensors[target]) ** 2.0)
                if len(violated[0]):
    ...
    def get_intermediate_variable_for_core(self):
        """
        @author: semercim
        :return:
            Rs
        ...
        for projection in range(self.num_ways):
            ...
            for mode in reversed(range(self.num_ways)):
                if projection == mode:
                    continue
                temp = np.kron(temp, self.projections[mode][:, :self.num_components[mode]])
            Rs.append(temp)
            Qs.append(np.dot(Rs[projection], Rs[projection].T))
        return Rs, Qs
    ...
    def update_projections(self, tensors_list, target_neighbors):
        ...
        for _ in range(self.update_iter):
            ...
            for mode in range(self.num_ways):
                ...
                _, Qs = self.get_intermediate_variable_for_core()
                T = Ts[mode]
                S = Ss[mode]
                Q = Qs[mode]
                Ws = []
                ...
                for index in range(num_instances):
                    if counts[index, index] != 0:
                        loss23 += counts[index, index] * np.dot(self.unfolded[mode][index],
                                                                np.dot(Q, self.unfolded[mode][index].T))
                    for index2 in range(index+1, num_instances):
                        if counts[index, index2] != 0:
                            inner_matrix_ = np.dot(self.unfolded[mode][index],
                                                   np.dot(Q, self.unfolded[mode][index2].T))
    ...
    def fit_tensor(self, tensors_list, labels, max_iter=None):
        ...
        if self.margin is None:
            self.margin = np.prod(self.num_components)

        super(CLMTD, self).fit_tensor(tensors_list, labels, max_iter)
        for mode in range(self.num_ways):
